...
const apiLimiter = rateLimit({ windowMs: 15 * 60 * 1000, max: 800 });
app.use(apiLimiter);
...
function saveExcelRows(rows) {
  ...
  const ws = XLSX.utils.json_to_sheet(normalized, { skipHeader: false });
...
function addJidToExcel(jid) {
  ...
  const exists = rows.some(
    (r) => String(r["WhatsApp JID"]).trim() === cleanJid,
  );
  if (exists) return;

  // Find first row where WhatsApp JID is empty
  const emptyIndex = rows.findIndex(
    (r) => !r["WhatsApp JID"] || String(r["WhatsApp JID"]).trim() === "",
  );

  if (emptyIndex !== -1) {
    // Fill that empty spot
    rows[emptyIndex]["WhatsApp JID"] = cleanJid;
  } else {
    // No empty row → push new
    rows.push({
      "WhatsApp JID": cleanJid,
      Contractors: "",
      "Individual Customers": "",
      Retailers: "",
    });
  ...
  saveExcelRows(rows);
  console.log("📥 Added new JID at correct position:", cleanJid);
}

function formatToJid(value) {
  const v = String(value).trim();

  // Already a JID
  if (v.endsWith("@s.whatsapp.net") || v.endsWith("@lid")) {
    return v;
  }

  // Only digits → convert to JID
  if (/^\d+$/.test(v)) {
...
function getUsersFromExcel(column) {
  const rows = getAllExcelRows();
  const columns = [
    "WhatsApp JID",
    "Contractors",
  ...
  // 🔹 "All" = return all values from all columns combined
  if (column === "All") {
    const allValues = [];
    ...
    rows.forEach((row) => {
      columns.forEach((col) => {
        ...
        if (value && String(value).trim() !== "") {
          allValues.push(formatToJid(value));
        }
      });
    });
  ...
  // 🔹 Individual column → return values from that column only
  return rows
    .filter((row) => row[column] && String(row[column]).trim() !== "")
    .map((row) => formatToJid(row[column]));
}

function getCountFromExcel(column) {
  const rows = getAllExcelRows();
  const columns = [
    "WhatsApp JID",
    "Contractors",
  ...
  // 🔹 "All" = total of all columns combined
  if (column === "All") {
    let total = 0;
  ...
  return rows.filter((row) => row[column] && String(row[column]).trim() !== "")
    .length;
}
...
app.get("/count", authMiddleware, (req, res) => {
  ...
  console.log("Count request for", req.query.target, "→", count);
  res.json({ count });
});
...
async function dailyJidHealthCheck(sock) {
  ...
  const users = getUsersFromExcel("All");
  for (const user of [...users]) {
    const alive = await isJidAlive(sock, user);
    if (!alive) console.log("⚠️ Dead JID (Excel not auto-removed):", user);
  }
...
app.post(
  ...
  async (req, res) => {
    ...
    if (!isConnected || !sock) {
      res.write(
        `data: ${JSON.stringify({ done: true, error: "WA not connected" })}\n\n`,
      );
      return res.end();
    }

    const users = getUsersFromExcel(target || "All");
    const total = users.length;
    if (!total) {
      res.write(
        `data: ${JSON.stringify({ done: true, error: "No users found" })}\n\n`,
      );
      return res.end();
    }
...
app.post("/broadcast/pause", authMiddleware, () => currentBroadcast?.pause());
app.post("/broadcast/resume", authMiddleware, () => currentBroadcast?.resume());
app.post("/broadcast/stop", authMiddleware, () => currentBroadcast?.stop());

// ====== START BOT ======
async function startBot() {
  const { state, saveCreds } = await useMultiFileAuthState("./auth");

  sock = makeWASocket({ auth: state, logger: Pino({ level: "fatal" }) });
  ...
  sock.ev.on("messages.upsert", async ({ messages, type }) => {
    ...
    // ✅ Save new user to Excel only
    if (isPrivateChat && !isAdmin && !msg.key.fromMe) {
      addJidToExcel(jid);
    }
...
Reply *MENU or HI* to open menu again.`,
    ...
    // (Menus + PDFs + replies remain SAME as your original code)
    // 👉 You already pasted them above – they continue to work unchanged
  });
}
